  ...
  return (
    <section id="menu" className="menu scroll-mt-10">
      <div className="custom-container py-10">
        ...
        {data.map(({attributes = {}, id = ''}, key) => {
            const { name = '', dish:dishes = [] } = attributes
            return (
                <div key={key} className="menu-group py-5">
                    <h1 className="mb-4 text-3xl font-bold text-red-700">{name}</h1>
                    <ul className="menu-list flex flex-wrap justify-center gap-5">
                        {dishes.map((dish, dKey) => <Dish key={dKey} data={{...dish,cmsPath}}/>)}
                    </ul>
                </div>
            )
        })}
